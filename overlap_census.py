...
def make_graphs(census_vec):
    for i in range(len(census_vec)):
        plt.figure(i)
        census = census_vec[i][0]
        overlap = census_vec[i][1]
        plt.hlines(0.25, 0, 31, colors='lightgray', linestyles='--')
        plt.hlines(0.5, 0, 31, colors='lightgray', linestyles='--')
        plt.hlines(0.75, 0, 31, colors='lightgray', linestyles='--')
        x = [k for k in census.keys()]
        y = [v / (len(overlap.nodes)) for v in census.values()]
        plt.fill(y, color=color[i], alpha=1)
        plt.gca().spines['right'].set_visible(False)
        plt.gca().spines['top'].set_visible(False)
        plt.plot(x, y, color=color[i])
        plt.yticks(ticks=[0.0, 0.25, 0.5, 0.75, 1.0], fontsize=20)
        plt.xticks(rotation=90, fontsize=20)
        plt.subplots_adjust(top=0.63, bottom=0.2, left=0.125, right=0.45)
        plt.xlabel('Network', fontsize=20)
        plt.ylabel('Overlap Census', fontsize=20)
    plt.show()
...
# Make colors vector to make the plots with different colors
com = [x for x in range(len(census_vec))]
...
# Call to the function that will plot every overlap census graph
make_graphs(census_vec)
